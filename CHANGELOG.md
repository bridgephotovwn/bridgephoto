...
- The size limit now drops the colour before it shrinks the page. A document almost never needs colour and dropping it saves a great deal, so a page stays readable at a far smaller size. This is what people chasing a portal's limit already do by hand.
- Split a batch at blank pages. Put a blank sheet between papers, photograph the whole pile in one go, and the app cuts it into separate documents — the way a desk scanner has worked for thirty years and no phone app does. The blank sheets are removed; the text already read follows each page into its new document, so nothing is recognised twice.
- Hide an ID number. India's own advice is not to hand out a plain photocopy of an Aadhaar card, and the official masked version covers the first eight digits. The app finds the number by its check digit — so an invoice reference is not mistaken for one — and draws the box for you to look at. **It never hides anything by itself**; you see exactly what would go and confirm it.
- Save the text of a scan as a **Word document or a spreadsheet**, not just plain text. Nine of the apps we looked at charge for this, and one allows a single file of 15 MB. It is expensive for them because they have to take a finished PDF apart again to work out what the words were; we never had to, because the text is already ours from reading the page. In the spreadsheet, columns are taken from where the page put them, and a part number like 0012 keeps its leading zero instead of being turned into 12.
